...
export function IndicationStrategyView({ products, indicationFilter }: IndicationStrategyViewProps) {
  ...
  return (
    <div className="space-y-6">
      {/* Header */}
      <div>
        <h2 className="text-2xl font-bold text-gray-900">Indication-Specific Strategy</h2>
        <p className="mt-1 text-sm text-gray-600">Analyze market dynamics and competitive positioning by indication</p>
      </div>
      ...
      <div className="grid grid-cols-2 gap-6">
        ...
        <div className="bg-white rounded-lg border border-gray-200 p-6">
          <h3 className="text-sm font-semibold text-gray-600 uppercase tracking-wide">Total Addressable Market</h3>
          <div className="mt-4">
            <div className="text-4xl font-bold text-gray-900">${indicationTAM}M</div>
            <p className="mt-2 text-sm text-gray-600">{indication?.label} market size</p>
          </div>
            ...
            {Object.entries(marketSize.byRegion).map(([region, percentage]) => (
              <div key={region} className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs font-semibold text-gray-600">{(percentage * 100).toFixed(0)}%</span>
                </div>
              </div>
            ))}
          </div>
        ...
        {/* Top Vendors List */}
        <div className="bg-white rounded-lg border border-gray-200 p-6">
          <h3 className="text-sm font-semibold text-gray-600 uppercase tracking-wide">Top Vendors</h3>
            ...
            {topVendorsInIndication.length > 0 ? (
              topVendorsInIndication.map((item, idx) => (
                <div key={idx} className="flex items-center justify-between">
                  ...
                  <span className="text-sm font-semibold text-gray-700">{item.share.toFixed(1)}%</span>
                </div>
              ))
            ) : (
              ...
              <div key={product.id} className="bg-white rounded-lg border border-gray-200 p-4">
                ...
                <div className="space-y-2 mt-4 pt-4 border-t border-gray-100">
                  <div className="flex justify-between">
                    ...
                    <span className="text-xs font-semibold text-gray-900">{product.share.toFixed(1)}%</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-xs text-gray-600">Regulatory</span>
                    <span className="text-xs font-semibold text-gray-900">{product.regulatory}</span>
                  </div>
                </div>
              </div>
            );
